...
const MOCK_RESPONSE = {
  query: "How does PacBio HiFi compare to Nanopore for structural variant detection?",
  answer:
  ...
  latency_ms: 1342.5,
  sources: [
    { pmid: "38291847", title: "Benchmarking long-read sequencing for structural variant detection in clinical genomics", year: "2024", authors: "Li H, Feng X, Chu C", score: 0.912, has_full: true },
    { pmid: "37105621", title: "Ultra-long Oxford Nanopore reads resolve complex structural variants in repeat-rich regions", year: "2023", authors: "Jain M, Koren S, Miga K", score: 0.887, has_full: true },
    { pmid: "36941834", title: "HiFi sequencing accuracy enables direct haplotype-resolved assembly of diploid genomes", year: "2023", authors: "Wenger A, Peluso P, Rowell W", score: 0.864, has_full: false },
    { pmid: "37842109", title: "Ensemble long-read SV calling improves sensitivity in population-scale genomics", year: "2023", authors: "Sedlazeck F, Lee H, Darby C", score: 0.841, has_full: true },
    { pmid: "38012745", title: "Nanopore sequencing of centromeric repeats reveals structural dynamics in cancer", year: "2024", authors: "Miga K, Koren S, Phillippy A", score: 0.798, has_full: false },
  ],
};
...
function ScoreBar({ score }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ fontFamily: "monospace", fontSize: 11, color, minWidth: 36 }}>{score.toFixed(3)}</span>
    </div>
  );
}
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
          ...
          <span style={{ fontSize: 11, color: "#64748b", padding: "2px 0" }}>{source.year}</span>
        </div>
      </div>
      <p style={{ margin: "0 0 8px", color: "#e2e8f0", fontSize: 13, lineHeight: 1.5, fontWeight: 500 }}>
        {source.title}
      </p>
      <p style={{ margin: "0 0 10px", color: "#64748b", fontSize: 12 }}>{source.authors}</p>
  ...
  return (
    <div style={{
	  ...
	  <ReactMarkdown
        components={{
          p: ({ children }) => (
            <p style={{ margin: "0 0 12px", color: "#cbd5e1", lineHeight: 1.75 }}>{children}</p>
          ),
          strong: ({ children }) => (
          ...
          li: ({ children }) => (
            <li style={{ color: "#cbd5e1", marginBottom: 4, lineHeight: 1.6 }}>{children}</li>
          ),
          code: ({ children }) => (
            <code style={{
              background: "#1e293b",
    ...
    <div style={{
      ...
      <div style={{
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", userSelect: "none" }}>
          <span style={{ fontSize: 12, color: "#475569", fontFamily: "monospace" }}>demo mode</span>
          <div
        ...
        {result && (
          <div>
            ...
            <div style={{
              ...
              query: {result.query}
              <span style={{ float: "right", color: "#334155" }}>{result.latency_ms}ms</span>
            </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {result.sources.map((s, i) => (
                  <SourceCard key={s.pmid} source={s} index={i} visible={sourcesVisible} />
                ))}
              </div>
            </div>
            ...
            {/* New query */}
            <button
              onClick={() => { setResult(null); setQuery(""); setTimeout(() => textareaRef.current?.focus(), 100); }}
              style={{
                marginTop: 28,
                background: "transparent",
                border: "1px solid #1e293b",
